...
def hide_window():
    ...
    menu = (item('Show', show_window), item('Check Now',
            reset_and_notify), item('Exit', quit_from_tray))
...
def load_data(year):
    url = f"https://api.cfl.ca/v1/games/{year}?key={api_key}"
...
def get_standings(year):
    url = f"https://api.cfl.ca/v1/standings/{year}?key={api_key}"
    standings = requests.get(url).json()
    standings_file = open("standings.json", "w")
    json.dump(standings, standings_file, ensure_ascii=False, indent=4)
...
class App:
    ...
    def pack_game(self):
        ...
        self.header = Label(root, textvariable=self.header_text,
                            font=("Leelawadee UI", 22, "bold"))
        self.teams = Label(root, textvariable=self.teams_text,
                           font=("Leelawadee UI", 20, "bold"))
        self.body = Label(root, textvariable=self.body_text,
        ...
        if len(live_now) > 0:
            self.score = Label(root, textvariable=self.score_text,
    ...
    def pack_standings(self):
        ...
        self.title = Label(self.table_frame_w,
                           text="Western Division", font=("Leelawadee UI", 16))
        ...
        for stand in standings['data']['divisions']['west']['standings']:
            ...
            w_l_t = str(stand['wins']) + " / " + \
                str(stand['losses']) + " / " + str(stand['ties'])
            # columns
            self.table_w['columns'] = ('position', 'team_name', 'w_l_t')
            self.table_w.column("#0", width=0, stretch=NO)
            self.table_w.column("position", anchor=CENTER, width=50)
            ...
            iid_var += 1
        iid_var = 0
        self.title = Label(self.table_frame_e,
                           text="Eastern Division", font=("Leelawadee UI", 16))
        ...
        for stand in standings['data']['divisions']['east']['standings']:
            ...
            self.table_e.column("position", anchor=CENTER, width=50)
    ...
    def __init__(self, master):
        ...
        self.show_standings_btn = Button(
            root, textvariable=self.sched_stand_toggle, command=self.toggle_standings).pack(in_=self.bottom)
        self.quit_btn = Button(root, text="Exit", command=quit_from_window).pack(
            in_=self.bottom, side=RIGHT)
        self.hide_btn = Button(root, text="Hide", command=hide_window).pack(
            in_=self.bottom, side=RIGHT)
        self.update_btn = Button(root, text="Update", command=reset_and_notify).pack(
            in_=self.bottom, side=RIGHT)
        self.get_labels()
